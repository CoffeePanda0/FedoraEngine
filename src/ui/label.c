...
FE_Label *FE_CreateLabel(char *text, int x, int y, SDL_Color color) // Creates new label, returns pointer to node
{
    char *ntext = 0;
    if (!text) {
        ntext = strdup(" ");
...
{
    // removes it from the LinkedLabel array, when found destroys the object.
    if (!l) {
