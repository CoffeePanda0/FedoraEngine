...
static void AddTile(int x, int y)
{
	...
	vec2 selectedtexture_position = FE_GetTexturePosition(map->r->atlas, (size_t)selected_tile);
...
static void Save_Action(Editor_Save_Config *config)
{
    ...
    FE_Map save = {
        ...
        .author = author,
        .gravity = gravity_f,
        .atlaspath = map->r->atlas->path,
        .atlas_tilesize = map->r->atlas->texturesize,
...
static void UI_Tile_Selector()
{
    ...
    size_t tilecount = (map->r->atlas->width / map->r->atlas->texturesize) * (map->r->atlas->height / map->r->atlas->texturesize);
    ...
    for (size_t i = 0; i < tilecount; i++) {
        SDL_Texture *t = FE_TextureFromAtlas(map->r->atlas, i);
        FE_UI_AddTile(g, t);
    }
...
void FE_Editor_Init(char *path)
{
    ...
    if (!path) {
        ...
        map->r = xmalloc(sizeof(FE_MapRenderer));
        map->r->atlas = FE_LoadResource(FE_RESOURCE_TYPE_ATLAS, "tileset.png");
        map->r->bg = 0;
        map->EndFlag = VEC_NULL;
        map->gravity = 70;
        map->name = 0;
        map->PlayerSpawn = VEC_NULL;
        map->r->static_bg = false;
        map->author = 0;
        map->tilecount = 0;
        map->tiles = 0;
        map->tilesize = 64;
